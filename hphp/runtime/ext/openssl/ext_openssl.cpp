...
bool HHVM_FUNCTION(openssl_private_decrypt, const String& data,
  ...
  case EVP_PKEY_RSA:
  case EVP_PKEY_RSA2:
    cryptedlen = RSA_private_decrypt(
      data.size(),
      (unsigned char *)data.data(),
      cryptedbuf,
      const_cast<RSA*>(EVP_PKEY_get0_RSA(pkey)),
      padding
    );
    if (cryptedlen != -1) {
      successful = 1;
...
bool HHVM_FUNCTION(openssl_private_encrypt, const String& data,
  ...
  case EVP_PKEY_RSA:
  case EVP_PKEY_RSA2:
    successful = (RSA_private_encrypt(
      data.size(),
      (unsigned char *)data.data(),
      cryptedbuf,
      const_cast<RSA*>(EVP_PKEY_get0_RSA(pkey)),
...
struct opensslExtension final : Extension {
  opensslExtension() : Extension("openssl", NO_EXTENSION_VERSION_YET, "hphp_hphpi") {}
